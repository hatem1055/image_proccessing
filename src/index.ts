...
async function returnCache(
  req: Request<Record<string, unknown>, Record<string, unknown>, Record<string, unknown>, QueryParams>,
  res: Response,
  next: NextFunction
) {
...
app.get("/api", async (req: Request<Record<string, unknown>, Record<string, unknown>, Record<string, unknown>, QueryParams>, res) => {
  const filename: string = req.query.filename;
  const height = Number(req.query.height);
  const width = Number(req.query.width);
  const outputFileName = `${filename}_${height}_${width}`;
  ...
  sharp(get_path(inputFileName, inDir))
    .resize({ height, width })
